/* 
 * drivers/input/touchscreen/hyn_cst8xx.c
 *
...
#include "hyn_cst816_ASJ_G1548FH107GG_MA1_A_fw.h"
...
2:the hyn_cst816_ASJ_G1548FH107GG_MA1_A_fw firmware will be updated in force.
...
static struct workqueue_struct *cst8xx_esd_workqueue;
#endif

#ifdef ANDROID_TOOL_SURPORT
static int cst8xx_firmware_info(struct i2c_client * client);
static int cst8xx_update_firmware(struct i2c_client * client, const unsigned char *pdata);
static  unsigned short g_unnormal_mode = 0;
static unsigned short g_cst8xx_tx = 9;
static unsigned short g_cst8xx_rx = 15;
#endif

static unsigned char  report_flag = 0;
static unsigned int   g_cst8xx_ic_version  = 0;
static unsigned int   g_cst8xx_ic_checksum = 0;

static struct workqueue_struct *cst8xx_wq;
...
static int cst8xx_boot_update_fw(struct i2c_client   *client,  unsigned char *pdata);
static unsigned char *pcst8xx_update_firmware = (unsigned char *)cst8_fw ; //the updating firmware
#endif
...
struct cst8xx_ts_data *g_ts=NULL;
...
#define TPD_X_REVER					1
#define TPD_Y_REVER					1

#define CST8XX_I2C_ADDR				0x1A
...
static int cst8xx_suspend(struct cst8xx_ts_data *ts);
static int cst8xx_resume(struct cst8xx_ts_data *ts);

#if defined(CONFIG_PM)
static int cst8xx_pm_suspend(struct device *dev);
static int cst8xx_pm_resume(struct device *dev);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
static void cst8xx_ts_early_suspend(struct early_suspend *h);
static void cst8xx_ts_late_resume(struct early_suspend *h);
#endif
...
static int cst8xx_i2c_read(struct i2c_client *client, unsigned char *buf, int len) 
{ 
	struct i2c_msg msg; 
	int ret = -1; 
	int retries = 0; 
...
Output:
    ...
    fail:	negative errno
*******************************************************/
static int cst8xx_i2c_read_register(struct i2c_client *client, unsigned char *buf, int len) 
{ 
...
static int cst8xx_i2c_write(struct i2c_client *client, unsigned char *buf, int len) 
{ 
	struct i2c_msg msg; 
	int ret = -1; 
	int retries = 0;
...
{ 
    ...
    ret = cst8xx_i2c_write(client, buf, 2);

    ret = cst8xx_i2c_read(client, buf, len);
	
    return ret; 
} 
#endif

static void cst8xx_reset_ic(unsigned int ms)
{
	...
	buf[0] = 0xD1;	
	buf[1] = 0x0E;	 	
	cst8xx_i2c_write(g_i2c_client, buf, 2);
#else
...
Output:
    ...
    fail:	negative
*******************************************************/
static int cst8xx_i2c_test(struct i2c_client *client)
{
	int retry = 0;
	int ret;
	unsigned char buf[4];
    ...
    if(retry==5) printk("linc cst8xx hyn I2C TEST error.ret:%d;\n", ret);
...
static irqreturn_t cst8xx_ts_irq_handler(int irq, void *dev_id)
{
	struct cst8xx_ts_data *ts=NULL;
	GTP_DEBUG_FUNC(); 
	ts=dev_id;
	if(!ts)
	{
	...
	queue_work(cst8xx_wq, &ts->work);
    cst8xx_irq_disable(ts);
	return IRQ_HANDLED;
}

static void cst8xx_touch_down(struct input_dev *input_dev,s32 id,s32 x,s32 y,s32 w)
{
	s32 temp_w = (w>>2);
#ifdef ICS_SLOT_REPORT
...
#define CST8XX_PROC_DIR_NAME	"cst1xx_ts"
#define CST8XX_PROC_FILE_NAME	"cst1xx-update"
...
static void cst8xx_close_fw_file(struct file * filp,mm_segment_t old_fs)
{
	set_fs(old_fs);
	if(filp)
...
static int cst8xx_read_fw_file(unsigned char *filename, unsigned char *pdata, int *plen)
{
	struct file *fp;
	mm_segment_t old_fs;
	int size;
...
clean:
	cst8xx_close_fw_file(fp, old_fs);
	return ret;
}


static int cst8xx_apk_fw_dowmload(struct i2c_client *client,
...
{	
		int ret=-1;	
		pcst8xx_update_firmware=(unsigned char *)pdata;
		ret = cst8xx_update_firmware(g_i2c_client, pdata);
		return ret;
}	
#endif
...
{	
	...
	printk(" linc cst8xx is entering cst8xx_proc_read_foobar.\n");
	...
	{		
		...
		buf[0] = 0xD1;
		buf[1] = 0x18;  
		ret = cst8xx_i2c_write(g_i2c_client, buf, 2);
		if(ret < 0)
		{				
		...
		for(read_count=0;read_count<data_len;read_count+=6){
			...
			ret = cst8xx_i2c_read(g_i2c_client, &buf[2+read_count], 6);
			if(ret < 0) 		
			{
		...
		ret = cst8xx_i2c_read(g_i2c_client, &buf[2], data_len);
		if(ret < 0) 		
		{
			printk("Read raw/diff data failed.error:%d.\n", ret);
			goto END;
...
static ssize_t cst8xx_proc_write_foobar(struct file *file, const char __user *buffer,size_t count, loff_t *data)
{
   ...
   printk(" linc cst8xx is entering cst8xx_proc_write_foobar .\n");
    
	if (copy_from_user(cmd, buffer, len))  {
		printk("linc cst8xxcopy data from user space failed.\n");
		return -EFAULT;
	}
	...
	if (cmd[0] == 0) {
	    ...
	    if(pdata == NULL) {
	        printk("linc cst8xxzalloc GFP_KERNEL memory fail.\n");
	        return -ENOMEM;
	    }
		ret = cst8xx_read_fw_file(&cmd[1], pdata, &length);
	  	if(ret < 0) {
			if(pdata != NULL) {
				kfree(pdata);
				pdata = NULL;	
		...
		ret = cst8xx_apk_fw_dowmload(g_i2c_client, pdata, length);
	  	if(ret < 0){
	        printk("linc cst8xxupdate firmware failed.\n");
			if(pdata != NULL) {
				kfree(pdata);
				pdata = NULL;	
			}	
		...
		cst8xx_firmware_info(g_i2c_client);    
...
static const struct proc_ops proc_tool_debug_proc_ops = {
        .proc_read      = cst8xx_proc_read_foobar,
        .proc_write     = cst8xx_proc_write_foobar,
};

static int  cst8xx_proc_fs_init(void)

{
   ...
   g_update_file = proc_create(CST8XX_PROC_FILE_NAME, 0777, g_proc_dir,&proc_tool_debug_proc_ops);
...
static int tp_info_version_read(struct seq_file *m, void *v)
{
	...
	fwver=g_cst8xx_ic_version;
	
    if ((fwver == 0xFF) || (fwver == 0x00))
              printk("get tp fw version fail!!\n"); 
    else
      sprintf(mtk_tp_version,"0x%02x,0x%02x",g_cst8xx_ic_version,g_cst8xx_ic_checksum);
...
static int proc_tp_version_open_cst8xx(struct inode *inode, struct file *file)
{
    return single_open(file, tp_info_version_read, NULL);
};

static const struct proc_ops tp_version_proc_fops1 = {
        .proc_open      = proc_tp_version_open_cst8xx,
        .proc_read     = seq_read,
};
...
static void cst8xx_esd_check_func(struct work_struct *);


void cst8xx_init_esd_protect(void)
{
	...
	INIT_DELAYED_WORK(&esd_check_work, cst8xx_esd_check_func);
	mutex_init(&esd_lock);


}
	
void cst8xx_esd_switch(s32 on)
{
	mutex_lock(&esd_lock);
	if (SWITCH_ESD_ON == on) {	/* switch on esd check */
		if (!esd_running) {
			...
			queue_delayed_work(cst8xx_esd_workqueue, &esd_check_work, esd_work_cycle);
		}
...
static void cst8xx_esd_check_func(struct work_struct *work)
{
	...
	while(retry++ < 3) {
		ret = cst8xx_i2c_read_register(g_i2c_client, buf, 1);
		if (ret > 0) break;
...
static int cst8xx_firmware_info(struct i2c_client * client)
{
	...
	buf[0] = 0xD2;
	buf[1] = 0x08;
	ret = cst8xx_i2c_read_register(client, buf, 8);
	if (ret < 0) return -1;	

	g_cst8xx_ic_version = buf[3];
	g_cst8xx_ic_version <<= 8;
	g_cst8xx_ic_version |= buf[2];
	g_cst8xx_ic_version <<= 8;
	g_cst8xx_ic_version |= buf[1];
	g_cst8xx_ic_version <<= 8;
	g_cst8xx_ic_version |= buf[0];

	g_cst8xx_ic_checksum = buf[7];
	g_cst8xx_ic_checksum <<= 8;
	g_cst8xx_ic_checksum |= buf[6];
	g_cst8xx_ic_checksum <<= 8;
	g_cst8xx_ic_checksum |= buf[5];
	g_cst8xx_ic_checksum <<= 8;
	g_cst8xx_ic_checksum |= buf[4];	

	printk("linc cst8xx [cst8xx] the chip ic version:0x%x, checksum:0x%x\r\n",
		g_cst8xx_ic_version, g_cst8xx_ic_checksum);

	if(g_cst8xx_ic_version==0xA5A5A5A5)
	{
		printk("linc cst8xx [cst8xx] the chip ic don't have firmware. \n");
		return -1;
	}
...
#define CST8XX_BIN_SIZE    (24*1024 + 24)

static int cst8xx_check_checksum(struct i2c_client * client)
{
	int ret;
	int i;
	...
	for(i=0; i<5; i++)
	{
		buf[0] = 0xA0;
		buf[1] = 0x00;
		ret = cst8xx_i2c_read_register(client, buf, 1);
		if(ret < 0)
		{
			mdelay(2);
			continue;
    ...
    if(buf[0]==0x01)
	{
		buf[0] = 0xA0;
		buf[1] = 0x08;
		ret = cst8xx_i2c_read_register(client, buf, 4);
        ...
        pData=(unsigned char  *)pcst8xx_update_firmware +24*1024+16;   //7*1024 +512
		...
		{
			printk("linc cst8xx hyn check sum error.\n");		
			return -1;
	...
	{
		printk("linc cst8xx hyn No checksum.\n");
		return -1;
	}	
	return 0;
}
static int cst8xx_into_program_mode(struct i2c_client * client)
{
	int ret;
	unsigned char buf[4];
	...
	ret = cst8xx_i2c_write(client, buf, 3);
	if (ret < 0)  return -1;
...
static int cst8xx_exit_program_mode(struct i2c_client * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst8xx_erase_program_area(struct i2c_client * client)
{
	int ret;
	unsigned char buf[3];
...
static int cst8xx_write_program_data(struct i2c_client * client,
		const unsigned char *pdata)
{
	int i, ret;
	...
	for (i=0; i<total_kbyte; i++) {
		...
		ret = cst8xx_i2c_write(client, i2c_buf, 4);
		if (ret < 0)
			goto error_out;
    	...
    	ret = cst8xx_i2c_write(client, temp_buf, 6);
    		if (ret < 0)
    			goto error_out;		
		}
#else
		...
		ret = cst8xx_i2c_write(client, i2c_buf, 1026);
		if (ret < 0)
			goto error_out;
#endif
		...
		i2c_buf[1] = 0x04;
		i2c_buf[2] = 0xEE;
		ret = cst8xx_i2c_write(client, i2c_buf, 3);
		if (ret < 0)
			goto error_out;
		...
		i2c_buf[0] = 0xA0;
		i2c_buf[1] = 0x05;
		ret = cst8xx_i2c_read_register(client, i2c_buf, 1);
		if (ret < 0)
			goto error_out;
...
static int cst8xx_update_firmware(struct i2c_client * client, const unsigned char *pdata)
{
	...
	printk("linc cst8xx----------upgrade cst8xx begain------------\n");
	
START_FLOW:	
	
	cst8xx_reset_ic(13);

	ret = cst8xx_into_program_mode(client);
	if (ret < 0) {
		printk("linc cst8xx[cst8xx]into program mode failed.\n");
		goto err_out;
	}

	ret = cst8xx_erase_program_area(client);
	if (ret) {
		printk("linc cst8xx[cst8xx]erase main area failed.\n");
		goto err_out;
	}

	ret = cst8xx_write_program_data(client, pdata);
	if (ret < 0) {
		printk("linc cst8xx[cst8xx]write program data into cstxxx failed.\n");
		goto err_out;
	}

    ret =cst8xx_check_checksum(client);
	if (ret < 0) {
		printk("linc cst8xx[cst8xx] after write program cst8xx_check_checksum failed.\n");
		goto err_out;
	}

	ret = cst8xx_exit_program_mode(client);
	if (ret < 0) {
		printk("linc cst8xx[cst8xx]exit program mode failed.\n");
		goto err_out;
	}

	cst8xx_reset_ic(10);
	
	printk("linc cst8xx hyn----------cst8xx_update_firmware  end------------\n");
...
static int cst8xx_update_judge( unsigned char *pdata, int strict)
{
	...
	for (i=0; i<(CST8XX_BIN_SIZE-4); i+=4) {
		fw_checksum += (*p);
		p++;
	}
	
	if (fw_checksum != (*p)) {
		printk("linc cst8xx[cst8xx]calculated checksum error:0x%x not equal 0x%x.\n", fw_checksum, *p);
		return -1;	//bad fw, so do not update
	}
	
	pBuf = &pdata[CST8XX_BIN_SIZE-16];
	...
	printk("linc cst8xx[cst8xx]the updating firmware:project_id:0x%04x,ic type:0x%04x,version:0x%x,checksum:0x%x\n",
			project_id, ic_type, fw_version, fw_checksum);

	if (strict > 0) {

		if(fw_checksum==g_cst8xx_ic_checksum){
				return -1;	
		}else{

			printk("linc cst8xx[cst8xx]the updating firmware:fw version(0x%x), the chip firmware:ic version(0x%x).\n",
			fw_version, g_cst8xx_ic_version);
			if (g_cst8xx_ic_version > fw_version) {
				return -1;	
			}
...
static int cst8xx_boot_update_fw(struct i2c_client   *client,  unsigned char *pdata)
{
	...
	if (flag == 1) {
		ret = cst8xx_update_judge(pdata, 1);
		if (ret < 0) {
			printk("linc cst8xx[cst8xx] no need to update firmware.\n");
			return 0;
		}
	}
	
	ret = cst8xx_update_firmware(client, pdata);
	if (ret < 0){
		printk("linc cst8xx [cst8xx] update firmware failed.\n");
		return -1;
	}
	...
	ret = cst8xx_firmware_info(client);
	if (ret < 0) {
		printk("linc cst8xx [cst8xx] after update read version and checksum fail.\n");
		return -1;
	}
...
static int hyn_boot_update_fw(struct i2c_client * client)
{
	...
	int ret;
	ptr_fw = pcst8xx_update_firmware;
	
	ret = cst8xx_boot_update_fw(client, ptr_fw);
    return ret;
...
static void cst8xx_touch_report(struct work_struct *work)
{
	...
	int  len_1, len_2;
#endif
	ts=container_of(work,struct cst8xx_ts_data,work);	
    ...
    if((hyn_ges_wakeup_switch == 1)&&(tpd_halt == 1)){
        ...
        buf[0] = 0xD0;
		buf[1] = 0x4C;
		err = cst8xx_i2c_read_register(g_i2c_client, buf, 1);
        if(err < 0)
        {
        	printk("linc cst8xxiic read gesture flag failed.\n");
        	goto OUT_PROCESS;
        }
		tmp_c=buf[0]&0x7F;
	...
	buf[0] = 0xD0;
	buf[1] = 0x00;
	ret = cst8xx_i2c_read_register(g_i2c_client, buf, 7);
	if(ret < 0) {
	...
	if(cnt == 0x01) {
		...
		if((buf[5]&0x80) == 0x80) //key
		{
			...
			for(idx=0; idx<i2c_len; idx+=6) {
    			...
    			ret = cst8xx_i2c_read_register(g_i2c_client, i2c_buf, len_2);
		...
		if ((buf[5]&0x80) == 0x80) {
			...
			buf[6] = 0x07;
			i2c_len = (cnt - 1)*5 + 3;
			ret = cst8xx_i2c_read_register(g_i2c_client, &buf[5], i2c_len);
			if (ret < 0)
				goto OUT_PROCESS;
			i2c_len += 5;
			key_status = buf[i2c_len - 3];
...
FINGER_PROCESS:
	...
	i2c_buf[1] = 0x00;
	i2c_buf[2] = 0xAB;
	ret = cst8xx_i2c_write(g_i2c_client, i2c_buf, 3);
	if(ret < 0) {
		printk("linc cst8xx hyn send read touch info ending failed.\r\n"); 
		cst8xx_reset_ic(20);
	}
	...
	for (i = 0; i < cnt; i++) {
        ...
        /*printk("linc cst8xx Point x:%d, y:%d, id:%d, sw:%d. \n", input_x, input_y, finger_id, sw);*/

		if (sw == 0x03) {
			cst8xx_touch_down(ts->input_dev, finger_id, input_x, input_y, input_w);
            cnt_down++;
        }
		else {
            cnt_up++;
            #ifdef ICS_SLOT_REPORT
			cst8xx_touch_up(ts->input_dev, finger_id);
            #endif
        }
		idx += 5;
	}
    ...
    if((cnt_up>0) && (cnt_down==0))
        cst8xx_touch_up(ts->input_dev, 0);
    #endif
...
KEY_PROCESS:
	...
	if(key_status&0x80) {
        ...
        if((key_status&0x7F)==0x03) {
			if((i==key_index)||(key_index==0xFF)) {
        		//cst8xx_touch_down(input_dev, 0, tpd_keys_dim_local[i][0], tpd_keys_dim_local[i][1], 50);
	...
	if(key_status&0x80) {
        ...
        if((key_status&0x7F)==0x03) {
			if((i==key_index)||(key_index==0xFF)) {
        		cst8xx_touch_down(ts->input_dev, 0, tpd_keys_dim_local[i][0], tpd_keys_dim_local[i][1], 50);
    			report_flag = 0xA5;
				key_index   = i;
			}
			else {
...
OUT_PROCESS:
	...
	buf[1] = 0x00;
	buf[2] = 0xAB;
	ret = cst8xx_i2c_write(g_i2c_client, buf, 3);
	if (ret < 0) {
...
{
    struct cst8xx_ts_data *ts_data = container_of(work,struct cst8xx_ts_data, resume_work);

    cst8xx_resume(ts_data);
}
/* frame buffer notifier block control the suspend/resume procedure */
struct notifier_block cst8xx_fb_notifier;
static int gtp_fb_notifier_callback(struct notifier_block *noti, unsigned long event, void *data)
{
	...
	if(!ts)
	{
		printk("cst8xx data pointor is null,no the device\n");
		return -ENODEV;
	}
	...
	if (ev_data && ev_data->data && event == FB_EVENT_BLANK) {
        ...
        if (*blank == FB_BLANK_UNBLANK) {
			...
			{
				printk("cst8xx ts pointer is null, resume fail\n");
			}
        }
    }
	...
	if (ev_data && ev_data->data && event == FB_EVENT_BLANK) {
        ...
        if (*blank == FB_BLANK_POWERDOWN) {
			...
			{
				printk("cst8xx ts pointer is null, suspend fail\n");
			}
		}
	}
...
static int cst8xx_pm_resume(struct device *dev)
{
	...
	return cst8xx_resume(ts);
}

/* bus control the suspend/resume procedure */
static const struct dev_pm_ops cst8xx_ts_pm_ops = {
	.suspend = cst8xx_pm_suspend,
	.resume = cst8xx_pm_resume,
};
...
static struct early_suspend cst8xx_early_suspend = {
	.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1,
	.suspend = cst8xx_ts_early_suspend,
	.resume = cst8xx_ts_late_resume,
};
#endif


static s32 cst8xx_request_irq(struct cst8xx_ts_data *ts)
{
	s32 ret = -1;
	ret = request_irq(ts->client->irq, cst8xx_ts_irq_handler, IRQ_TYPE_EDGE_RISING, ts->client->name, ts);
	if (ret) {
...
static s8 cst8xx_request_input_dev(struct cst8xx_ts_data *ts)
{
	s8 ret = -1;
...
#if   defined(CONFIG_FB)
		INIT_WORK(&ts->resume_work,cst8xx_ts_resume_work);
		ts->fb_notif.notifier_call = gtp_fb_notifier_callback;
		fb_register_client(&ts->fb_notif);
#elif defined(CONFIG_HAS_EARLYSUSPEND)
		ts->early_suspend.suspend=cst8xx_ts_early_suspend;
		ts->early_suspend.resume=cst8xx_ts_late_resume;
...
static s32 cst8xx_request_io_port(struct cst8xx_ts_data *ts)
{
	s32 ret = 0;
...
static int cst8xx_ts_get_dt_coords(struct device *dev, char *name,struct cst8xx_ts_platform_data *pdata)
{
...
static int cst8xx_parse_dt(struct device *dev,struct cst8xx_ts_platform_data *pdata)
{
    ...
    np = dev->of_node;
	ret = cst8xx_ts_get_dt_coords(dev, "hynitron,panel-coords", pdata);
	if (ret && (ret != -EINVAL))
		return ret;
	ret = cst8xx_ts_get_dt_coords(dev, "hynitron,display-coords", pdata);
	if (ret)
		return ret;
	...
	{
		printk("CST8XX Invalid GPIO, get reset-gpio failed\n");
		return pdata->reset_gpio;
	}
	pdata->irq_gpio = of_get_named_gpio_flags(np, "hynitron,irq-gpio",0, &pdata->irq_gpio_flags);
	if (pdata->irq_gpio < 0)
	{
		printk("CST8XX Invalid GPIO, get irs-gpio failed\n");
...
{	
	...
	if (client->dev.of_node) {
		pdata = devm_kzalloc(&client->dev,sizeof(struct cst8xx_ts_platform_data), GFP_KERNEL);
		if (!pdata) {
			dev_err(&client->dev,"cst8xx Failed to allocate memory for pdata\n");
			return -ENOMEM;
		}

		ret = cst8xx_parse_dt(&client->dev,pdata);
		if (ret)
			return ret;
	} else {
	...
	if (!pdata) {
		dev_err(&client->dev, "cst8xx invalid pdata\n");
		return -EINVAL;
	}
	...
	ret = cst8xx_i2c_test(client);
	if (ret < 0) {
		printk("linc cst8xx hyn i2c communication failed.\n");
		return -1;
	}
	
	mdelay(20);

	ret = cst8xx_request_io_port(ts);
	if (ret < 0) {
		printk("cst8xx request IO port failed.");
		return ret;
	}


	ret = cst8xx_request_input_dev(ts);
	if (ret < 0) {
		printk("cst8xxrequest input dev failed");
	}

	g_input_dev=ts->input_dev;

	ret = cst8xx_request_irq(ts);
	if (ret < 0) {
		printk("cst8xx works in polling mode.");
	} else {
		printk("cst8xx works in interrupt mode.");
	}
	...
	ret = cst8xx_update_firmware(client, pcst8xx_update_firmware);
	if (ret < 0){
		printk("linc cst8xx [cst8xx] update firmware failed.\n");
		return -1;
	}
	...
	if(ret < 0){
		printk("linc cst8xx hyn_boot_update_fw failed.\n");
		return -1;
	}
	...
	ret = cst8xx_firmware_info(client);
	if(ret < 0){
		printk("linc cst8xx cst8xx_firmware_info failed.\n");
		return -1;
	}
#endif	
	...
	if(ret < 0) {
		printk("linc cst8xx hyn create cst8xx proc fs failed.\n");
	}
	...
	cst8xx_esd_workqueue = create_singlethread_workqueue("cst2xx_esd_workqueue");
	if (cst8xx_esd_workqueue == NULL)
		printk("linc cst8xxcreate cst2xx_esd_workqueue failed!");
	...
	cst8xx_init_esd_protect();
	cst8xx_esd_switch(SWITCH_ESD_ON);
...
static void cst8xx_enter_sleep(struct i2c_client *client)
{
	int ret;
	int retry = 0;
	unsigned char buf[2];
...
static int cst8xx_resume(struct cst8xx_ts_data *ts)
{
    ...
    if(hyn_ges_wakeup_switch == 1){
		...
		buf[1] = 0x4C;
		buf[2] = 0x00;
		cst8xx_i2c_write(g_i2c_client, buf, 3);
        tpd_halt=0;
		hyn_lcd_flag = 1;	
	...
	printk(" linc cst8xx wake up done.\n");
...
static int cst8xx_suspend(struct cst8xx_ts_data *ts)
{
	...
	printk(" linc cst8xx enter sleep.\n");
	...
	cst8xx_esd_switch(SWITCH_ESD_OFF);
    ...
    cst8xx_enter_sleep(g_i2c_client);
	
	printk(" linc cst8xx enter sleep done.\n");
...
{
	printk(" linc hyn is entering cst8xx_ts_init.\n");

	cst8xx_wq = create_singlethread_workqueue("cst8xx_wq");
	if (!cst8xx_wq) {
...
{
	printk(" linc hyn is entering cst8xx_ts_exit.\n");
